...
for (nt in c(250, 500, 750, 1000, 1250, 1500)) { # vary n_T
  ...
  for (time in 1:sim) {
    ...
    results_df = rbind(results_df, 
                       c(nt, "SR", mse_sr[time]),
                       c(nt, "DR", mse_dr[time]),
                       c(nt, "ACW", mse_acw[time]),
                       c(nt, "COKE", mse_coke[time]))
}}

colnames(results_df) = header
save(results_df, file = paste("./", "changeN_seed", seedflag, ".Rdata", sep = ""))
